...
import { Upload, Download, Eye, Lock, FileText, Music, AlertCircle, CheckCircle, Loader } from 'lucide-react';
...
const AudioStego = () => {
  ...
  const handleCoverAudioChange = (e) => {
  ...
  const handleEmbed = async () => {
    if (!coverAudio) {
      setError('Please select a cover audio file');
      return;
    }
    ...
    try {
      ...
      const response = await axios.post('/encode/audio', formData, {
        headers: {
  ...
  const handleExtract = async () => {
    if (!coverAudio) {
      setError('Please select a stego audio file');
      return;
    }
    if (!password) {
      setError('Please enter a password');
    ...
    try {
      const formData = new FormData();
      formData.append('stego_audio', coverAudio);
      formData.append('password', password);

      const response = await axios.post('/decode/audio', formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="text-center mb-8">
        <div className="flex items-center justify-center mb-4">
          ...
          <h1 className="text-3xl font-bold text-gray-900">Audio Steganography</h1>
        </div>
        <p className="text-gray-600 text-lg">
          Hide and extract secret data in audio files using LSB (Least Significant Bit) technique
        </p>
      </div>
        ...
        <div className="card">
          ...
          <div className="mb-6">
            <label className="block text-sm font-medium text-gray-700 mb-2">
              {mode === 'embed' ? 'Cover Audio' : 'Stego Audio'}
            </label>
            <div className="file-drop-zone">
              <input
                ...
                type="file"
                accept=".wav"
                onChange={handleCoverAudioChange}
                className="hidden"
              />
              <button
              ...
              >
                {coverAudio ? (
                  <div className="flex items-center justify-center space-x-2 text-green-600">
                    ...
                    <span className="font-medium">{coverAudio.name}</span>
                  </div>
                ) : (
                  <div className="flex flex-col items-center space-y-2 text-gray-500">
                    <Upload className="w-8 h-8" />
                    <span>Click to upload audio</span>
                    <span className="text-sm">WAV format supported</span>
                  </div>
                )}
              </button>
            </div>
